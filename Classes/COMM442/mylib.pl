...
# &make_select(), wordlist()

sub wordlist() {
  my $file, @list, %words, @nwords, @retlist;
  $file = $_[0];
  $file =~ s#.*/(.*)$#$1#;           #SECURITY: No path
  $file = "text/$file";              #SECURITY: Add a specific path
  ...
  foreach (@list){
     ...
     foreach (split){
       s/\W//g;
       if (!/\w/){next;}      #Has to be alphanumeric    
       $words{$_}++;          #Hash makes words unique
     }
  }
  ...
  foreach (@words){
     push (@nwords, "$words{$_}\t$_<br>\n");
  }
  @retlist= (sort numeric_reverse(@nwords));
  return @retlist;
}
...
sub get_lines{
  my @array;
  $file = $_[0];
  $file =~ s#/##g;                  # SECURITY! Current directory only. Try w4.5.pl /etc/passwd
  open (FH, $file);
  @array= <FH>;
  unshift (@array,"$file\n\n");
  close (FH);
